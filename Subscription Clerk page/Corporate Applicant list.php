...
<html>
<head>
    <title>Corporate Applicant list</title>
        ...
        <ul>
            ...
            <li class='active has-sub'><a href='' style="background:#FFCC33;"><span>Approve Applicants</span></a>
                <ul>
                    ...
                    <li class='last'><a href='Corporate Applicant list.php' style="background:#FFCC33;"><span>Corporate</span></a></li>
                </ul>
            </li>
            ...
            <article class="topcontent">
                <content>
                    ...
                    if(!empty($applicantNum)){
                        $query = "update corporateApplicant set approvalStatus='".$status."' , approvalDateTime=SYSDATE() where applicantNum=".$applicantNum.";";
                    ...
                    <table id="mytable" cellspacing="0" summary="Individual Applicant List">
                        <caption>Corporate Applicant List :<br></caption>
                        <tr>
                            <th scope="col" abbr="Application Number">Application Number</th>
                            <th scope="col" abbr="Applicant Name">Company Name</th>
                            <th scope="col" abbr="Applicantion Status">Application Status</th>
                            <th scope="col" abbr="Approval Date/Time">Approval Date/Time</th>
                        </tr>
<?php
$query = "select applicantNum,companyName,approvalStatus,approvalDateTime from corporateApplicant order by (approvalStatus='New') DESC, approvalStatus ASC;";
...
if (mysqli_num_rows($result) > 0) {
    ...
    while($row = mysqli_fetch_assoc($result)) {
        if($i%2==0){
        echo "<tr>"
                ."<td><form method='POST' action='CorporateApprove.php'><input type='submit' style= 'background: none; border: none; color: blue; text-decoration: underline; cursor: pointer;' name='applicantNum' value='", $row["applicantNum"], "'/></form></td>"
        ...
        . "</tr>";
         ...
         echo "<tr>"
                ."<td class='alt'><form method='POST' action='CorporateApprove.php'><input type='submit' style= 'background: none; border: none; color: blue; text-decoration: underline; cursor: pointer;' name='applicantNum' value='", $row["applicantNum"], "'/></form></td>"
                ."<td class='alt'>".$row["companyName"]."</td>"
